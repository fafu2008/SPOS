...
class CalculaterController: UIViewController , UICollectionViewDataSource , UICollectionViewDelegateFlowLayout {
    ...
    var cal_type = 0 //收银还是改价计算类型 0 收银 1 改价
    ...
    override func viewDidLoad() {
        ...
        //增加改价抹零监听
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(self.recNotification(_:)), name: "change_price", object: nil)
    }
    
    func recNotification(notification: NSNotification)
    {
        if (notification.name == "change_price")
        {
            vPayInfo.hidden = true
            vPayType.hidden = true
            cal_type = 1
            cvCalculater.reloadData()
        }
    }
    
    override func viewWillAppear(animated: Bool) {
        super.viewWillAppear(animated)
        self.navigationController?.navigationBar.overlayColor = BASE_UICOLOR
                                    ...
                                    if let data = value["data"] as? String {
                                        ...
                                        do {
                                            ...
                                            if payWayList.count > 0 {
                                                ...
                                                let orders = self!.cdPayWay.selectObject("Order") as! [Order]
                                                /*
                                                let order1:Order = orders[0]
                                                print(order1.ord_no)*/
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if let controller = segue.destinationViewController as? QRCodeOrScanController {
            ...
            controller.amount = DefaultKit.change_string_to_int(self.lblNeedPay.text!)
            controller.or_amount = DefaultKit.change_string_to_int(self.lblConsume.text!)
            controller.disc_amount = DefaultKit.change_string_to_int(self.lblPreferential.text!)
            controller.order_type = pmt_tag
            
            if (pmt_tag == "Alipay" ||
                pmt_tag.hasPrefix("Alipay")){
                controller.title = "支付宝"
                controller.style = getPayStyle()
            }else if (pmt_tag == "Weixin" ||
                pmt_tag.hasPrefix("Weixin")) {
                controller.title = "微信支付"
                controller.style = getPayStyle()
            }else if (pmt_tag == "Baifubao" ||
                pmt_tag.hasPrefix("Baifubao")) {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        if collectionView == cvCalculater {
            ...
            lblTitle.text = payWay.pmt_internal_name
            return cell
        }
    }
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        if collectionView == cvCalculater {
            ...
            case 7:
                ...
                {
                    let ran = (text as NSString).rangeOfString(".")
                    if (ran.length > 0)
                    {
                        text = (text as NSString).substringToIndex(ran.location)
                        cachedResult = text
                    }
                }
            case 12:
            ...
            if payWay.pmt_tag == "Cash" {
                ...
                //print(self.lblConsume,self.lblPreferential,self.lblNeedPay)
                
                PayManager.doPay("Cash",
                            ord_name: "现金收银",
                            original_amount: DefaultKit.change_string_to_int(self.lblConsume.text!),
                            discount_amount: DefaultKit.change_string_to_int(self.lblPreferential.text!),
                            ignore_amount: 0,
                            trade_amount: DefaultKit.change_string_to_int(self.lblNeedPay.text!), auth_code: nil, order_no : nil,callback: {[weak self] (easValue , message) in
                    let data1 = easValue.dataUsingEncoding(NSUTF8StringEncoding)
                    
                    do {
                        if let dic1 : [String : AnyObject] = try NSJSONSerialization.JSONObjectWithData(easValue.dataUsingEncoding(NSUTF8StringEncoding)!, options: .AllowFragments) as? [String : AnyObject] {
                            if let status = dic1["status"] {
                                if (status as! Int == 1)
                                {
