...
@property (weak, readwrite, nonatomic) UILabel *disCash;
...
@property (strong,nonatomic) NSString *deft_string;
...
+ (instancetype)numberPadWithDelegate:(id<APNumberPadDelegate>)delegate withFrame:(CGRect)frame displayLable:(UILabel*)label{
    return [self numberPadWithDelegate:delegate numberPadStyleClass:nil withFrame:frame displayLable:(UILabel*)label];
}

+ (instancetype)numberPadWithDelegate:(id<APNumberPadDelegate>)delegate numberPadStyleClass:(Class)styleClass withFrame:(CGRect)frame displayLable:(UILabel*)label{
    return [[self alloc] initWithDelegate:delegate numberPadStyleClass:styleClass withFrame:frame displayLable:(UILabel*)label];
}

- (instancetype)initWithDelegate:(id<APNumberPadDelegate>)delegate numberPadStyleClass:(Class)styleClass withFrame:(CGRect)frame displayLable:(UILabel*)label{
    self = [super initWithFrame:CGRectZero];
    if (self) {
        self.disCash = label;
        self.deft_string = self.disCash.text;
        self.styleClass = styleClass;
        self.frame = frame;//[self.styleClass numberPadFrame];
        ...
        [self.leftButton addTarget:self action:@selector(functionDomainButtonAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)numberButtonAction:(UIButton *)sender {
    ...
    */
    ...
    if (self.disCash.text.length > 15)
    {
        return;
    }
    ...
    if (_delegateFlags.textInputSupportsShouldChangeTextInRange) {
        ...
        if ([self.disCash.text isEqualToString:@"请输入金额"] ||
            [self.disCash.text hasPrefix:@"可转账"] ||
            [self.disCash.text hasPrefix:@"可提现"])
        {
            self.disCash.text = @"";
        }
        
        self.disCash.textColor = [UIColor blackColor];
        self.disCash.text = [self.disCash.text stringByAppendingString:text];
    }
}

-(void)functionDomainButtonAction
{
    if (!self.disCash)
    {
        return;
    ...
    for (int i = 0; i < [self.disCash.text length]; i++) {
        
        //NSString *str = [NSString stringWithFormat:@"%hu",[self.disCash.text characterAtIndex:i]];
        unichar c = [self.disCash.text characterAtIndex:i];
    ...
    self.disCash.textColor = [UIColor blackColor];
    self.disCash.text = [self.disCash.text stringByAppendingString:@"."];
}
...
- (void)clearButtonAction {
     ...
     */
    if (!self.disCash ||
        (self.disCash && [self.disCash.text hasPrefix:@"可转账"]) ||
        (self.disCash && [self.disCash.text hasPrefix:@"可提现"]) ||
        (self.disCash && [self.disCash.text isEqualToString:@"请输入金额"]) ||
        (self.disCash && (self.disCash.text.length == 0)))
    {
        return;
    }
    ...
    if (_delegateFlags.textInputSupportsShouldChangeTextInRange) {
        ...
        self.disCash.text = [self.disCash.text substringToIndex:self.disCash.text.length - 1];
        
        if (self.disCash.text.length == 0)
        {
            self.disCash.text = self.deft_string;
            self.disCash.textColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3];
        }
